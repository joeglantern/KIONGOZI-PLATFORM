...
export default function ChatScreen() {
  ...
  const shareConversation = async () => {
    try {
      ...
      const conversationTitle = currentConversation?.title || 'Kiongozi AI Conversation';
      const timestamp = new Date().toLocaleDateString();

      let conversationText = `${conversationTitle}\nGenerated on ${timestamp}\n\n`;

      messages.forEach((message, index) => {
        if (index === 0 && message.text.includes('Habari!')) {
          // Skip the welcome message for cleaner export
          return;
        }
        const sender = message.isUser ? 'You' : 'Kiongozi AI';
        conversationText += `${sender}: ${message.text}\n\n`;
      });
